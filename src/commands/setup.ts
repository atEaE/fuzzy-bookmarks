 ...
 * @returns void
 */
export function setupExecute(config: IFzbConfig): void {
  vscode.window
    .showInputBox({
      prompt: `Create a file "${config.defaultBookmarkFullPath()}" as setup. If you want to continue, enter "y|yes".`,
    })
    .then(input => {
        ...
        try {
          ...
          if (fs.existsSync(fileutils.resolveHome(config.defaultBookmarkFullPath()))) {
            ...
            vscode.window.showInformationMessage(
              `OK! Create a new destination folder(${config.defaultBookmarkFullPath()}).`,
            );
          }
